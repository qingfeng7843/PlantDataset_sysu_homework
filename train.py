...
def train_val(model, loader_train, loader_val, opt, criterion, epoch, csv_file):
    ...
    with open(csv_file, mode='w', newline='') as file:
        ...
        writer.writerow(['Epoch', 'Train_Loss', 'Train_Acc', 'Val_Loss', 'Val_Acc'])  # 写入表头
        ...
        for e in range(epoch):
            ...
            # 将每个epoch的结果写入CSV文件
            writer.writerow([e, loss_t, acc_t, loss_v, acc_v])
...
def train(e, model, loader, opt, criterion):
    ...
    for i, (img, target) in enumerate(tqdm_loader):
        ...
        feature, x = model(img)
        output = torch.sigmoid(x).float()  # 进行前向传播并通过sigmoid激活函数获得预测概率
